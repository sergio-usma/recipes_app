module RecipesHelper
  ...
  def details_buttons(recipe)
    ...
    if can?(:create, recipe)
      content << (link_to 'Generate shopping list', general_shopping_lists_url,
                          class: 'buttons btn btn-primary auto-width').to_s
      content << (link_to 'Add ingredient', new_recipe_food_path(recipe),
                          class: 'buttons btn btn-primary auto-width').to_s
    end
    content.html_safe
  end
  ...
  def foods(current_recipe)
    recipe_foods_list = current_recipe.recipe_foods
    foods = []
    recipe_foods_list.each { |recipe_food| foods << Food.where(id: recipe_food.food_id) }

    new_foods = []
    foods.each do |food|
      recipe = recipe_foods_list.filter { |elem| elem.food_id == food[0].id }
